...
saveRDS(numNAsTraining, "./numNAsTraining.RDS") ## saving object
saveRDS(numNAsTesting, "./numNAsTesting.RDS") ## saving object
...
readRDS("./numNAsTraining.RDS") ## loading saved objects
...
```
## 3. Training models:
- So far we have the test set ready-tidy to start applying machine learning. So let's partitioning the data... 60% - 20% - 20%
...
- It takes a lot of time to train a random forest model on a data set as big as this (rows = `r nrow(trainingAll)` x cols = `r ncol(trainingAll)`). I tryed it for around 30 minutes with parallelizing for allowing the use of three of the cores of a Intel® Core™ i7-6500U CPU @ 2.50GHz × 4 with 8GB of RAM availables, but couldn't see the work ended and some issues appears with the closing of conections of the cluster for paralleilzation.
So, I tried rpart as method, and yields just 0.5073 of accuracy, not enougth to pass the quizz. Tried lda as method, but yields just 0.7 of accuracy, not enougth. Also svm with a accuracy of 0.89. It's not so bad, but random forest is better.

- I choose to split the dataset in 4 folds aiming to reduce the training random forest models time, and then stack them all four together with a random forest again.
...
```
- Now, following the instructions of Mentor Len Greski on <https://github.com/lgreski/datasciencectacontent/blob/master/markdown/pml-randomForestPerformance.md>, my objetive it's to speed up the process:
...
```
-- Parallelizing again (not neccesary to make it again step by step):
